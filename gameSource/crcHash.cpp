...
unsigned int crcHash( unsigned char *inKey, unsigned int inKeyLength ) {
    ...
    // this ensures that different-length all-0 keys hash to different
    // values
    unsigned long hash = inKeyLength;
    
    for( i=0; i<inKeyLength; ++i ) {
        hash = (hash >> 8) ^ crctab[ (hash & 0xff) ^ inKey[i] ];
        }
