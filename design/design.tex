...
After the UN inspector's move (which may not occur during each round) accumulated diamonds are given to the high bidder in each region.  If more than one unit bids in a region, both players pay their bids, but only the winner receives the accumulated diamonds.  In the case of a tie, both players still pay, but neither get diamonds, and the diamonds in that region accumulate into the next round.
